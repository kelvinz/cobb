...
- `skills/compact/`: compact `tasks/memory.md` by summarising older entries when tracking files get noisy.
...
- `tasks/memory.md`
  - Living "what matters" record: project definition, current state, decisions, milestones, gotchas.
...
4. `review` (when needed): run a quality check before finalise.
   Memory: capture durable risks/follow-ups inline when needed.
5. `commit` (`finalise` mode): prepare tracking updates (archive completed PRD and update `tasks/memory.md` when needed).
...
6. `compact` (periodic): summarise older memory entries to keep tracking files easy to scan.
