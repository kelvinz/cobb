...
- `skills/review/`: strict decision review in either `pr` mode or `local` mode, with optional durable memory capture and a git fallback if `gh` is unavailable.
...
7. `review` (`pr` mode, when needed): run a quality check before finalise.
   Memory: capture durable risks/follow-ups inline when needed.
8. `commit` (`finalise` mode): prepare tracking updates (archive completed PRD, apply `tasks/todo.md` completion strikethrough, and update `tasks/memory.md` when needed).
   Create one pre-merge finalise commit only when tracking changes exist, then merge using the repository-approved strategy (merge/squash/rebase), close branch, and delete the completed feature branch safely.
