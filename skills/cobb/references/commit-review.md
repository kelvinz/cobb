...
4. Preserve each commit's original intent and message; revise a message only when needed to describe its repaired contents accurately. If a repair depends on later work and cannot fit its proposed commit, regroup it or ask for a follow-up commit rather than leaving an intermediate commit broken.
