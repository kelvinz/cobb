...
2. Prompt:
   - `0`: proceed to finalise without optional suggestions on a feature branch, or conclude review on the default branch
   - `1..N`: implement selected suggestions; accept multi-select replies
   - `N+1`: implement all suggestions
   - `N+2`: stop without finalising
   - show one **Recommended reply** with reasoning; use `0` when suggestions are genuinely optional, or the relevant numbered selection when it materially reduces security, correctness, data-loss, or near-term operational risk
